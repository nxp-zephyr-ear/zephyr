...
{
	/* With a core clock of 260MHz, we need a minimum wait of 13
	 * cycles to satisfy requirement. Wait for 130 as this is
	 * the same value MCUX SDK uses.
	 */
				    ...
				    const struct mipi_dbi_config  *dbi_config)
{
	...
	/* Save pixel format */
	if (dbi_config->pixfmt == MIPI_DCS_PIXEL_FORMAT_16BIT) {
		data->pixel_fmt = LCDIC_DATA_FMT_HALFWORD;
	} else if (dbi_config->pixfmt == MIPI_DCS_PIXEL_FORMAT_8BIT) {
		data->pixel_fmt = LCDIC_DATA_FMT_BYTE;
	} else {
