...
static int flash_flexspi_nor_read_id(const struct device *dev,
		uint8_t *vendor_id)
{
 ...
 */
static int flash_flexspi_nor_quad_enable(const struct device *dev,
 ...
 */
static int flash_flexspi_nor_4byte_enable(const struct device *dev,
					uint32_t (*flexspi_lut)[MEMC_FLEXSPI_CMD_PER_SEQ],
					uint32_t en4b)
{
	struct flash_flexspi_nor_data *data = dev->data;
 ...
 */
static int flash_flexspi_nor_config_flash(const struct device *dev,
			...
			uint32_t (*flexspi_lut)[MEMC_FLEXSPI_CMD_PER_SEQ])
{
	...
	ret = flash_flexspi_nor_4byte_enable(dev, flexspi_lut, dw16.enter_4ba);
	if (ret == 0) {
	...
	if (jesd216_bfp_read_support(&header->phdr[0], bfp,
		...
		ret = flash_flexspi_nor_quad_enable(dev, flexspi_lut, dw15.qer);
		if (ret < 0) {
			return ret;
		}
...
static int flash_flexspi_nor_check_jedec(const struct device *dev,
			uint32_t (*flexspi_lut)[MEMC_FLEXSPI_CMD_PER_SEQ])
{
	struct flash_flexspi_nor_data *data = dev->data;
	int ret;
	...
	ret = flash_flexspi_nor_read_id(dev, (uint8_t *)&vendor_id);
	if (ret < 0) {
		return ret;
	}
	...
	case 0x25C2:
		...
		return flash_flexspi_nor_quad_enable(dev, flexspi_lut, JESD216_DW15_QER_VAL_S1B6);
	default:
		return -ENOTSUP;
	}
}
...
static int flash_flexspi_nor_probe(const struct device *dev)
{
	struct flash_flexspi_nor_data *data = dev->data;
	 ...
	 * in this driver
	 */
	ret = flash_flexspi_nor_check_jedec(dev, flexspi_lut);
	if (ret == 0) {
		/* Flash was supported, SFDP probe not needed */
		goto _program_lut;
	}

	ret = flash_flexspi_nor_sfdp_read(dev, 0, sfdp_buf, sizeof(sfdp_buf));
	if (ret < 0) {
		goto _exit;
	}
	...
	ret = flash_flexspi_nor_sfdp_read(dev,
	...
	/* Configure flash */
	ret = flash_flexspi_nor_config_flash(dev, header, bfp, flexspi_lut);
	if (ret < 0) {
		goto _exit;
	}
...
static int flash_flexspi_nor_init(const struct device *dev)
{
	...
	/*
	 * Copy controller pointer to device structure in RAM to avoid
	 * flash access in critical section
	 */
	memcpy(&data->controller, data->ctrl_ref, sizeof(data->controller));
	if (!device_is_ready(&data->controller)) {
		LOG_ERR("Controller device is not ready");
		return -ENODEV;
	}

	if (flash_flexspi_nor_probe(dev)) {
		if (memc_flexspi_is_running_xip(&data->controller)) {
...
#define FLASH_FLEXSPI_NOR(n)						\
	static struct flash_flexspi_nor_data				\
		...
		.ctrl_ref = DEVICE_DT_GET(DT_INST_BUS(n)),		\
